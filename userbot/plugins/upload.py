        ...
        if not caption_rts.lower().endswith(".mp4"):
            await event.client.send_file(
                ...
                progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                    progress(d, t, udir_event, itime, "Uploading...", caption_rts)
                ),
            )
        else:
...
@icssbot.on(admin_cmd(pattern="upload (.*)", outgoing=True))
@icssbot.on(sudo_cmd(pattern="upload (.*)", allow_sudo=True))
...
@icssbot.on(admin_cmd(pattern="uploadf (.*)", outgoing=True))
@icssbot.on(sudo_cmd(pattern="uploadf (.*)", allow_sudo=True))
async def uploadir(event):
    input_str = "".join(event.text.split(maxsplit=1)[1:])
    path = Path(input_str)
    start = datetime.now()
...
@icssbot.on(admin_cmd(pattern="circle ?(.*)", outgoing=True))
@icssbot.on(sudo_cmd(pattern="circle ?(.*)", allow_sudo=True))
async def video_icsf(event):
    ...
    if input_str:
        ...
        if not os.path.exists(path):
            ...
            return
        icse = await edit_or_reply(event, "`Converting to video note..........`")
    ...
    if os.path.exists(PATH):
        ...
        await event.client.send_file(
            ...
            progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                progress(d, t, icse, itime, "Uploading...", PATH)
            ),
        )
        os.remove(PATH)
    await icse.delete()
