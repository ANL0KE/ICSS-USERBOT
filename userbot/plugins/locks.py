...
@icssbot.on(admin_cmd(pattern=r"lock (.*)"))
@icssbot.on(sudo_cmd(pattern=r"lock (.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@icssbot.on(admin_cmd(pattern="unlock (.*)"))
@icssbot.on(sudo_cmd(pattern="unlock (.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    input_str = event.pattern_match.group(1)
...
@icssbot.on(admin_cmd(pattern="locks$"))
@icssbot.on(sudo_cmd(pattern="locks$", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    res = ""
