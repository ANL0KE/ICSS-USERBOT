...
async def inline_handler(event):
    try:
        ...
        query = event.text
        me = await bot.get_me()
        if query.startswith("yt") and event.query.user_id == bot.uid:
...
@tgbot.on(events.callbackquery.CallbackQuery(data=re.compile(b"audio", from_users=OWNER_ID))
async def _(sur):
...
@tgbot.on(events.callbackquery.CallbackQuery(data=re.compile(b"video", from_users=OWNER_ID))
async def _(fuk):
